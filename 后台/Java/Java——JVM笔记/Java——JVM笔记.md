<center><h1>Java——JVM笔记</h1></center>
# Java内存区域

![Java内存区域](Java——JVM笔记.assets/1567931965144.png)

- 方法区：存放类信息、常量、static修饰的静态属性、JIT（及时编译）代码，是数据共享的区域，会出现内存溢出问题。

- 堆区：存放实例对象、GC（垃圾回收器），是数据共享的区域，会出现内存溢出问题。

- 虚拟机栈：Java方法在运行时的内存模型，是数据不共享的区域，会出现内存溢出问题。

  ![虚拟机栈内存模型](Java——JVM笔记.assets/1567932967613.png)

- 本地方法栈：同虚拟机栈差不多，不同的是，本地方法栈服务的对象是JVM执行的native方法，而虚拟机栈服务的是JVM执行的java方法。

- 程序计数器：Java线程的私有数据，这个数据就是执行下一条指令的地址，是数据不会共享的区域，不会出现内存溢出问题。

# Java内存模型

Java内存模型（JMM）是一种抽象的规范。

![Java内存模型](Java——JVM笔记.assets/1567933956030.png)

读写方式：线程从工作空间读取数据，如果工作区间没有数据，就从主内存中将数据读取到工作空间，线程再从工作空间读取数据；线程往工作空间中写数据时，先判断工作区间是否有数据，如果有，修改工作空间数据，工作空间将修改数据更新到主内存中，如果工作区间没有数据，将从主内存中先将数据读取到工作空间，然后修改工作空间数据，最后工作空间将修改的数据更新到主内存中。

分为以下3个部分

1. 主内存：存放共享的数据。

2. 工作内存：存放私有的数据，如果是基本类型，直接分配到工作内存；如果是引用类型，工作内存中存放的是引用地址，引用的对象存放在堆中。

3. 工作方式：

   A. 当线程修改私有数据时，直接修改工作空间的数据即可。

   B. 当线程修改共享数据时，需要把数据复制到工作空间中，在工作空间中修改数据，修改完成后刷新到主内存中。
...
12. HashMap原理、CurrentHashMap、分段锁、红黑树（一种特殊的二叉树）
...
14. 线程池
    ...
    - ExecutorService
    - Executors
    - 创建线程池分类：创建固定个数的线程池、创建可变数的线程池、创建可调度的线程池
    - ThreadPoolExecutor
    - 线程池的状态：Running、Shutdown、Stop、Tidying、Terminated

# Java 类的加载机制

> 该部分内容摘抄自：[纯洁的微笑-Jvm 系列(一):Java 类的加载机制](http://www.ityouknow.com/jvm/2017/08/19/class-loading-principle.html)
...
![img](Java——JVM笔记.assets/jvm-1.png)
...
![img](Java——JVM笔记.assets/class.png)
...
![img](Java——JVM笔记.assets/calssloader.png)
